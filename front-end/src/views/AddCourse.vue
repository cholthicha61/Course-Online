      ...
      <div class="flex items-center ml-13">
        <div id="image-preview-1"
          ...
          <div v-for="(item, i) in course.images" :key="i" class="relative">
            ...
            <img :src="item.result" :alt="'Image Preview ' + (i + 1)" style="max-width: 100%; height: auto" />
            <button @click="removeImage(i)" class="absolute -top-2 right-0 bg-black text-white p-1 rounded-full">
              X
            </button>
          </div>
        </div>
