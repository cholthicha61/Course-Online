<template>
  <div class="container mx-auto px-96 mt-8">
    ...
    <div class="mb-4 flex items-center">
      ...
      <select id="category" v-model="course.category" class="w-3/4 p-2 border border-gray-300 rounded" @change="categoryChangeHandler">
    ...
    <div class="mb-2 flex items-center">
      ...
      <div class="flex items-center ml-13">
        <div id="image-preview-1" class="max-w-sm p-6 mb-4 bg-gray-100 border-dashed border-2 border-gray-400 rounded-lg items-center mx-auto text-center cursor-pointer mr-4 overflow-scroll-y">
          ...
          <div v-for="(item, i) in course.images" :key="i" class="relative">
            ...
            <button @click="removeImage(i)" class="absolute -top-2 right-0 bg-black text-white p-1 rounded-full">X</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex justify-center py-5 mb-8">
      <button @click="submitCourse" class="bg-sky-700 text-white px-9 py-2 rounded hover:shadow-xl hover:bg-sky-800">Submit</button>
      <div class="w-4"></div>
      <button @click="cancel" class="bg-gray-500 text-white px-9 py-2 rounded hover:shadow-xl hover:bg-gray-500">Cancel</button>
    </div>
  </div>
</template>
