        ...
        <v-sheet class="ma-3 rounded-border">
          ...
          >
            <v-img
              v-if="item.course"
              height="200px"
              :src="`${img}/${item.course.courseImage}`"
              cover
            ></v-img>
            <v-card-text>
              <h1
              ...
              <div class="box-description mt-1">
                <p
                  v-if="
                    ...
                    item.course.description.length === 0
                  "
                ></p>
                <p v-else>
                  <template
              ...
              <div class="status mt-4 size-auto w-auto">
                <v-card
                  v-if="item.status === 'Incourse'"
                  class="status d-flex justify-center align-center h-6 incourse"
                >
                  {{ item.status }}
                </v-card>
                <v-card
                  v-else-if="item.status === 'Waiting'"
                  class="status d-flex justify-center align-center h-6 waiting"
                >
                  {{ item.status }}
                </v-card>
                <v-card
                  v-else-if="item.status === 'Canceled'"
                  class="status d-flex justify-center align-center h-6 canceled"
                >
                  {{ item.status }}
                </v-card>
              </div>
